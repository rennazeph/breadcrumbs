# Regression Checklist — Backpack 1.31.11
...
## v1.31.11 Quick Notes recovery checks

- Resize a Quick Note down to the minimum. Confirm the `🔗` header button can still open the board-level Links Inspector.
- Open saved links, then shrink/move the note. Confirm the inspector remains usable and has its own close button.
- Use `Reset size` from the inspector. Confirm note content and saved links are preserved.
- Import an old Quick Notes board with very small/offscreen notes. Confirm notes are clamped to at least 4×3 and visible within the board.
- Use `Repair Layout`. Confirm no notes are deleted and saved links remain attached to their notes.
- Press Escape while the Links Inspector is open. Confirm it closes.
