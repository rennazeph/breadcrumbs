# The Backpack 1.31.11 — Split Build
...
Version **1.31.11** is a Quick Notes safety hotfix on top of the v1.31.10 CSS contract build. It moves permanent link administration out of resizable note cards, adds layout repair controls, and keeps small/offscreen notes recoverable.
...
| Quick Notes | Fixed-grid post-it board | Stored in `appState.quickNotes`; supports board-only export/import. Pasted links are captured into each note's saved `links` list and administered through a board-level Links Inspector. |
...
- The saved-links UI is board-level instead of inside the note card, so it remains reachable when cards are small or reset.
- The board has a `Repair Layout` action and the Links Inspector has `Reset size` for the selected note.
