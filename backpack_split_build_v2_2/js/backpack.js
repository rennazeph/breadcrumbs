/***************************************************************************
    ...
    const BP_USER_CONFIG = {
      ...
      contentLabels: {
        notes: "up to four Markdown documents",
    ...
    const defaultState = {
      ...
      placeholders: {
        ...
        CUSTOM_1: "",
        CUSTOM_2: "",
        HEADER_QUOTE: "Markdown notes and a visual card workspace"
      },
      ui: {
        editingEventId: null,
        darkMode: true,
    ...
    const BP_NOTES_MAX_DOCUMENTS = 4;

    function markdownTitleFromFileName(fileName, fallbackIndex = 1) {
      const stem = String(fileName || '')
        .replace(/\.(?:md|markdown)$/i, '')
        .replace(/[_-]+/g, ' ')
        .replace(/\s+/g, ' ')
        .trim();
      if (!stem) return `Note ${fallbackIndex}`;
      return stem.slice(0, 48);
    }

    function createEmptyNotesDocument(index = 1) {
      return {
        id: uid('note'),
        title: `Note ${index}`,
        markdown: '',
    ...
    function normalizeNotesState(notes) {
      ...
      let documents = Array.isArray(input.documents) ? input.documents.slice(0, BP_NOTES_MAX_DOCUMENTS) : [];

      // Migrate the pre-1.31.12 single-document shape.
      const legacyMarkdown = String(input.markdown || '');
      const legacySource = String(input.source || 'Upload required');
      const onlyDefaultEmpty = documents.length === 1 && !documents[0]?.markdown && !documents[0]?.fileName;
      if (legacyMarkdown && (!documents.length || onlyDefaultEmpty)) {
        const fileName = legacySource.replace(/^Uploaded\s+/i, '').trim();
        documents = [{
          id: documents[0]?.id || uid('note'),
          title: markdownTitleFromFileName(fileName, 1),
          markdown: legacyMarkdown,
          source: legacySource,
          fileName: /\.(?:md|markdown)$/i.test(fileName) ? fileName : ''
        }];
      }

      documents = documents.map((document, index) => ({
        id: document?.id || uid('note'),
        title: String(document?.title || markdownTitleFromFileName(document?.fileName, index + 1) || `Note ${index + 1}`).slice(0, 48),
        markdown: String(document?.markdown || ''),
        source: String(document?.source || 'Upload required'),
        fileName: String(document?.fileName || '')
      }));

      if (!documents.length) documents.push(createEmptyNotesDocument(1));
      const activeId = documents.some(document => document.id === input.activeId)
        ? input.activeId
        : documents[0].id;
    ...
    function loadState() {
      try {
        ...
        loaded.notes = normalizeNotesState(loaded.notes);
        loaded.quickNotes = normalizeQuickNotesBoard(loaded.quickNotes);
        if (!["notes", "quicknotes"].includes(loaded.activeTab)) loaded.activeTab = "notes";
        loaded.ui.promotedWidget = "";
    ...
    function getCoreWorkspaceState() {
      const activeTab = ["notes", "quicknotes"].includes(appState.activeTab) ? appState.activeTab : "notes";
      return {
        version: BP_USER_CONFIG.appVersion,
        activeTab,
        notes: normalizeNotesState(appState.notes),
        quickNotes: normalizeQuickNotesBoard(appState.quickNotes),
        placeholders: {
          HEADER_QUOTE: String(appState.placeholders?.HEADER_QUOTE || "Markdown notes and a visual card workspace")
        },
        ui: {
          darkMode: Boolean(appState.ui?.darkMode),
          readingDark: Boolean(appState.ui?.readingDark),
          density: appState.ui?.density === "readable" ? "readable" : "compact",
          theme: getThemeConfig(appState.ui?.theme).id,
          displayMenuOpen: Boolean(appState.ui?.displayMenuOpen),
          dataMenuOpen: Boolean(appState.ui?.dataMenuOpen)
        }
      };
    }
    ...
    function saveState() {
      ...
      localStorage.setItem(BP_USER_CONFIG.storageKey, JSON.stringify(getCoreWorkspaceState()));
    }

    function exportState() {
      const payload = {
        format: "backpack-v2-core",
        exportedAt: new Date().toISOString(),
        app: "The Backpack",
        state: getCoreWorkspaceState()
      };
    ...
    async function importStateFile(file) {
      try {
        ...
        appState.notes = normalizeNotesState(appState.notes);
        appState.quickNotes = normalizeQuickNotesBoard(appState.quickNotes);
        if (!["notes", "quicknotes"].includes(appState.activeTab)) appState.activeTab = "notes";
        appState.ui.promotedWidget = "";
        appState = normalizeUIState(appState);
        saveState();
        renderApp();
        showToast("Backpack workspace imported.");
      } catch (error) {
        console.error(error);
    ...
    /***************************************************************************
     * Backpack v2 core tab registry
     * Backpack v2 intentionally exposes only Notes and Drawing Board.
     * New core tools should be added deliberately through this registry.
     ***************************************************************************/
    const tabs = [];
    ...
    [
      ...
      { id: "quicknotes", label: "Drawing Board", icon: "🗂️", group: "workflow", render: renderQuickNotes, bind: bindQuickNotes }
    ].forEach(registerTab);
    ...
    /***************************************************************************
     * Notes module
     * Up to four uploaded Markdown documents are retained as lightweight subtabs.
     ***************************************************************************/
    function getActiveNotesDocument() {
      appState.notes = normalizeNotesState(appState.notes);
      return appState.notes.documents.find(document => document.id === appState.notes.activeId)
        || appState.notes.documents[0];
    }

    function renderFileButton(inputId, label, accept, { multiple = false } = {}) {
      return `<label class="bp-file-button">⬆ ${escapeHTML(label)}<input id="${inputId}" type="file" accept="${accept}"${multiple ? ' multiple' : ''} /></label>`;
    }
    ...
    async function loadNotesDocuments(files) {
      const incoming = Array.from(files || []).slice(0, BP_NOTES_MAX_DOCUMENTS);
      ...
      for (const file of incoming) {
        ...
        if (!target && appState.notes.documents.length < BP_NOTES_MAX_DOCUMENTS) {
          target = createEmptyNotesDocument(appState.notes.documents.length + 1);
        ...
        target.title = markdownTitleFromFileName(file.name, appState.notes.documents.indexOf(target) + 1);
      ...
      saveState();
      renderApp();
      const skippedMessage = skippedCount ? ` ${skippedCount} file${skippedCount === 1 ? '' : 's'} skipped because all four tabs are occupied.` : '';
      showToast(`${loadedCount} Markdown note${loadedCount === 1 ? '' : 's'} loaded.${skippedMessage}`);
    }

    async function replaceActiveNotesDocument(file) {
      if (!file) return;
      appState.notes = normalizeNotesState(appState.notes);
      const active = getActiveNotesDocument();
      active.markdown = await file.text();
      active.fileName = file.name;
      active.title = markdownTitleFromFileName(file.name, appState.notes.documents.indexOf(active) + 1);
    ...
    function addEmptyNotesDocument() {
      appState.notes = normalizeNotesState(appState.notes);
      if (appState.notes.documents.length >= BP_NOTES_MAX_DOCUMENTS) {
        showToast('Notes already has four document tabs.');
        return;
      }
      const document = createEmptyNotesDocument(appState.notes.documents.length + 1);
    ...
    function closeActiveNotesDocument() {
      appState.notes = normalizeNotesState(appState.notes);
      const active = getActiveNotesDocument();
      ...
      if (appState.notes.documents.length === 1) {
        appState.notes.documents[0] = createEmptyNotesDocument(1);
        appState.notes.activeId = appState.notes.documents[0].id;
      } else {
        appState.notes.documents.splice(index, 1);
        const next = appState.notes.documents[Math.min(index, appState.notes.documents.length - 1)];
        appState.notes.activeId = next.id;
      }
      saveState();
      renderApp();
    }
    ...
    function renderNotes() {
      ...
      const canAdd = appState.notes.documents.length < BP_NOTES_MAX_DOCUMENTS;
      return `
        <section class="bp-panel bp-notes-panel">
          <div class="bp-panel-header bp-notes-header">
            ...
            <span class="bp-pill">${appState.notes.documents.length}/${BP_NOTES_MAX_DOCUMENTS}</span>
          </div>
          ...
          <div class="bp-notes-toolbar">
            ${renderFileButton('bpNotesFiles', 'Load Markdown', '.md,.markdown,text/markdown,text/plain', { multiple: true })}
            ${hasMarkdown ? renderFileButton('bpReplaceNotesFile', 'Replace Current', '.md,.markdown,text/markdown,text/plain') : ''}
          ...
          <div class="bp-window bp-notes-window">
            ...
            <article class="bp-window-body bp-markdown" role="tabpanel">
              ${hasMarkdown ? renderMarkdown(parsePlaceholders(active.markdown)) : `
                ...
                <p>Markdown is cached locally in Backpack state. No local server or neighboring-file check is used.</p>
              `}
            </article>
          </div>
        </section>
    ...
    function bindNotes() {
      ...
      $('#bpAddNotesTab')?.addEventListener('click', addEmptyNotesDocument);
      $('#bpNotesFiles')?.addEventListener('change', event => {
        const files = event.target.files;
        if (files?.length) loadNotesDocuments(files);
        event.target.value = '';
      });
      $('#bpReplaceNotesFile')?.addEventListener('change', event => {
        const file = event.target.files?.[0];
        if (file) replaceActiveNotesDocument(file);
        event.target.value = '';
      });
      $('#bpCloseNote')?.addEventListener('click', closeActiveNotesDocument);
    }
    ...
    function exceedsOverlap(candidate, existing) {
      ...
      // One-row cards must remain individually reachable; do not permit them to
      // hide under another note even though expanded notes allow one-cell overlap.
    ...
    function clampQuickNoteGeometry(note) {
      const cfg = BP_USER_CONFIG.quickNotes;
      const before = `${note.col},${note.row},${note.w},${note.h},${note.collapsed},${note.expandedH}`;
      ...
      return before !== `${note.col},${note.row},${note.w},${note.h},${note.collapsed},${note.expandedH}`;
    }
    ...
    function findQuickNoteSpaceForSize(w, h, ignoreId = null) {
      ...
      const safeH = Math.max(cfg.collapsedH, Math.min(cfg.rows, Math.round(Number(h) || cfg.defaultH)));
    ...
    function repairQuickNotesLayout({ resetSizes = false } = {}) {
      ...
      appState.quickNotes.notes.forEach((note, index) => {
        ...
        if (!canPlaceNote(note, note.id)) {
          const fallbackH = note.collapsed ? cfg.collapsedH : cfg.defaultH;
          const safeSpot = findQuickNoteSpaceForSize(note.w, note.h, note.id)
            || findQuickNoteSpaceForSize(cfg.defaultW, fallbackH, note.id);
    ...
    function resetQuickNoteSize(noteId) {
      ...
      if (!note) return false;
      const candidate = { ...note, collapsed: false, w: cfg.defaultW, h: cfg.defaultH, expandedH: cfg.defaultH };
    ...
    function toggleQuickNoteCollapsed(noteId) {
      const cfg = BP_USER_CONFIG.quickNotes;
      const note = appState.quickNotes.notes.find(item => item.id === noteId);
      if (!note) return false;
      if (!note.collapsed) {
      ...
      const candidate = { ...note, collapsed: false, h: desiredH, expandedH: desiredH };
      clampQuickNoteGeometry(candidate);
      if (canPlaceNote(candidate, note.id)) {
        Object.assign(note, candidate, { updatedAt: new Date().toISOString() });
        return true;
      }
      const safeSpot = findQuickNoteSpaceForSize(note.w, desiredH, note.id);
      if (safeSpot) {
        Object.assign(note, safeSpot, { collapsed: false, expandedH: desiredH, updatedAt: new Date().toISOString() });
        return true;
      }
    ...
    function addQuickNote() {
      ...
      if (!spot) return showToast('No available Drawing Board space for a new note.');
      ...
      appState.quickNotes.notes.push({
        ...
        title: 'Note',
        collapsed: false,
        expandedH: BP_USER_CONFIG.quickNotes.defaultH,
        text: 'New note',
    ...
    function renderQuickNotes() {
      ...
      const collapsedCount = appState.quickNotes.notes.filter(note => note.collapsed).length;
      return `
        <section class="bp-panel bp-qn-panel">
          <div class="bp-panel-header bp-qn-header">
            ...
            <div class="bp-qn-actions" aria-label="Drawing Board actions">
              <span class="bp-pill">${appState.quickNotes.notes.length} notes · ${collapsedCount} cards</span>
              <button type="button" id="bpRepairQuickNotes" title="Recover small/offscreen cards without deleting content">Repair Layout</button>
          ...
          <details class="bp-qn-guide">
            ...
            <span>Expanded notes move from bottom chrome and resize with ↘.</span>
            <span>Collapsed cards move from anywhere except the subtle + button.</span>
            <span>🔗 opens the board-level saved-links inspector.</span>
            <span>Grid: ${cfg.columns}×${cfg.rows}; overlap limit: ${cfg.maxOverlapCells} cell for expanded notes.</span>
          </details>
        </section>
      `;
    }
    ...
    function renderQuickNote(note) {
      ...
      const narrow = Number(note.w) <= 2;
      return `
        <article class="bp-qn-note${collapsed ? ' bp-qn-note-collapsed' : ''}${narrow ? ' bp-qn-note-narrow' : ''}${linksOpen ? ' bp-qn-note-links-selected' : ''}" data-note-id="${note.id}"${collapsed ? ` data-qn-collapsed-drag="${note.id}"` : ''} style="left:${left}%;top:${top}%;width:${width}%;height:${height}%;z-index:${note.z};--bp-qn-note-bg:${escapeHTML(note.color)};--bp-qn-note-text:${escapeHTML(note.textColor || '#151515')};--bp-qn-title-indent:${titleIndent};">
          <div class="bp-qn-note-header">
            ${collapsed
              ? `<strong class="bp-qn-collapsed-title">${escapeHTML(note.title || 'Note')}</strong>`
              : `<input class="bp-qn-title" data-qn-title="${note.id}" value="${escapeHTML(note.title || 'Note')}" aria-label="Drawing Board note title" title="Edit note title" />
            ...
            <button type="button" class="bp-qn-collapse-toggle" data-qn-collapse-toggle="${note.id}" title="${collapsed ? 'Expand note' : 'Collapse to title card'}" aria-label="${collapsed ? 'Expand note' : 'Collapse note'}" aria-expanded="${!collapsed}"></button>
          </div>
          ${collapsed ? '' : `
    ...
    function exportQuickNotes() {
      const payload = {
        ...
        type: "drawingBoard",
        version: BP_USER_CONFIG.appVersion,
        drawingBoard: appState.quickNotes
      };
      const blob = new Blob([JSON.stringify(payload, null, 2)], { type: "application/json" });
      const url = URL.createObjectURL(blob);
      const a = document.createElement("a");
      a.href = url;
      a.download = `backpack-drawing-board-${toDateKey(new Date())}.json`;
      a.click();
      URL.revokeObjectURL(url);
    ...
    function normalizeQuickNotesBoard(board) {
      ...
      safeBoard.notes = Array.isArray(safeBoard.notes) ? safeBoard.notes.map((note, index) => {
        ...
        const normalized = {
          ...
          expandedH: Number.isFinite(Number(note.expandedH)) ? Number(note.expandedH) : Math.max(BP_USER_CONFIG.quickNotes.minH, Number(note.h) || BP_USER_CONFIG.quickNotes.defaultH),
    ...
    async function importQuickNotesFile(file) {
      try {
        const text = await file.text();
        const parsed = JSON.parse(text);
        const incoming = parsed.drawingBoard || parsed.quickNotes || parsed.state?.quickNotes || parsed;
        if (!incoming || !Array.isArray(incoming.notes)) throw new Error("Invalid Drawing Board file.");
        ...
        console.error(error);
        showToast("Could not import that Drawing Board file.");
      }
    }
    ...
    function bindQuickNotes() {
      ...
      $$('[data-qn-collapse-toggle]').forEach(button => {
        ...
        button.addEventListener('click', event => {
          ...
          commitQuickNoteBodies({ extractLinks: true });
          if (!toggleQuickNoteCollapsed(button.dataset.qnCollapseToggle)) return;
          if (appState.quickNotes.openLinksNoteId === button.dataset.qnCollapseToggle) appState.quickNotes.openLinksNoteId = '';
          saveState();
          renderApp();
        });
      });
      $$('[data-qn-collapsed-drag]').forEach(card => {
        card.addEventListener('pointerdown', event => {
          ...
          startQuickNotePointer(event, card.dataset.qnCollapsedDrag, 'move');
        });
      });
    ...
    function renderQuickNotesSoft() {
      ...
      appState.quickNotes.notes.forEach(note => {
        ...
        if (el) {
          el.style.setProperty('--bp-qn-note-bg', note.color);
          el.style.setProperty('--bp-qn-note-text', note.textColor || '#151515');
        }
      });
    }
    ...
    function startQuickNotePointer(event, noteId, mode) {
      ...
      function onMove(moveEvent) {
        ...
        if (canPlaceNote(candidate, note.id)) {
          ...
          element.classList.toggle('bp-qn-note-narrow', Number(note.w) <= 2);
        }
      }
    ...
    function applyGlobalUIState() {
      ...
      if (displayBtn) {
        ...
        setToolbarButtonState(displayBtn, {
          ...
          pressed: isOpen,
          active: isOpen || appState.ui.density === 'readable'
        });
      }
