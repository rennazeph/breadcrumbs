...
The Backpack day starts at `07:00`. Expected arrival now defaults to `07:30`.

Rules:

- `07:30` is on time by default.
- `07:31` is late by default.
...
The selected-day frame remains structural, but hover is intentionally quieter so the calendar can be used in high-contrast Lime Analog and Grayscale themes without the active cell becoming a large filled square.


## v1.31.10 Calendar/Gantt theme note

Calendar and Event Gantt now have a semantic colour contract: `--bp-calendar-day-bg`, `--bp-calendar-day-hover-bg`, `--bp-gantt-empty-bg`, and `--bp-priority-*` variables. New themes should update those tokens instead of styling Calendar/Gantt selectors directly.
