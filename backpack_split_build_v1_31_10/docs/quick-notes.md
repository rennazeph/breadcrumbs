...
- The `🔗` footer button opens the note's saved link drawer.
...
The `🔗` drawer is the administrative surface for those permanent links. Opening the drawer shows the saved URLs for that note. Deleting from the drawer actually removes the stored link.
