/***************************************************************************
    ...
    function renderQuickNoteLinksPanel(note) {
      const links = normalizeQuickNoteLinks(note.links || []);
      const isOpen = appState.quickNotes.openLinksNoteId === note.id;
      if (!isOpen) return '';
      return `
        <div class="bp-qn-links-panel" data-qn-links-panel="${note.id}">
          ...
          ` : `<p>No links saved yet. Paste a link in this note to capture it here.</p>`}
        </div>
      `;
    }
    ...
    function renderQuickNote(note) {
      ...
      return `
        <article class="bp-qn-note${linksOpen ? ' bp-qn-note-links-open' : ''}" data-note-id="${note.id}" style="left:${left}%;top:${top}%;width:${width}%;height:${height}%;z-index:${note.z};background:${escapeHTML(note.color)};color:${escapeHTML(note.textColor || '#151515')};--bp-qn-title-indent:${titleIndent};">
          ...
          ${renderQuickNoteLinksPanel(note)}
