(() => {
  ...
  /***************************************************************************
   * Backpack 2.4 core configuration
   ***************************************************************************/
  ...
  function bindDrawingBoard() {
    ...
    $$('[data-qn-body]').forEach(element => {
      ...
      element.addEventListener("input", () => {
        const note = appState.drawingBoard.notes.find(item => item.id === element.dataset.qnBody);
        if (persistBody(note, element, { captureLinks: true })) queueSaveState();
      });
      ...
      element.addEventListener("blur", () => {
        const note = appState.drawingBoard.notes.find(item => item.id === element.dataset.qnBody);
        const changed = persistBody(note, element, { captureLinks: true, sanitizeElement: true });
  ...
  async function importWorkspace(file) {
    try {
      ...
      saveState();
      renderApp();
      showToast("Backpack workspace imported and upgraded to v2.4.");
    } catch (error) {
      console.error(error);
